...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class Advection extends ShaderPass {
      ...
      updateWithOptions(options: { dt?: number; isBounce?: boolean; BFECC?: boolean }) {
        if (!this.uniforms) return;
        if (typeof options.dt === 'number') this.uniforms.dt.value = options.dt;
        if (typeof options.isBounce === 'boolean') this.line.visible = options.isBounce;
    ...
    class ExternalForce extends ShaderPass {
      ...
      initWithProps(simProps: Record<string, unknown>) {
        super.init();
      ...
      updateWithOptions(props: { mouse_force?: number; cellScale?: THREE.Vector2; cursor_size?: number }) {
    ...
    class Viscous extends ShaderPass {
      ...
      updateWithOptions(options: { viscous?: number; iterations?: number; dt?: number }) {
        ...
        for (let i = 0; i < iter; i++) {
          ...
          fbo_out = (i % 2 === 0 ? this.props.output1 : this.props.output0) as THREE.WebGLRenderTarget;
    ...
    class Divergence extends ShaderPass {
      ...
      updateWithOptions(options: { vel?: THREE.WebGLRenderTarget }) {
    ...
    class Poisson extends ShaderPass {
      ...
      updateWithOptions(options: { iterations?: number }) {
        let p_out: THREE.WebGLRenderTarget | null = null;
        const iter = options.iterations ?? 0;
        for (let i = 0; i < iter; i++) {
          const p_in = i % 2 === 0 ? this.props.output0 : this.props.output1;
          p_out = (i % 2 === 0 ? this.props.output1 : this.props.output0) as THREE.WebGLRenderTarget;
          if (this.uniforms) this.uniforms.pressure.value = (p_in as THREE.WebGLRenderTarget).texture;
    ...
    class Pressure extends ShaderPass {
      ...
      updateWithOptions(options: { vel?: THREE.WebGLRenderTarget; pressure?: THREE.WebGLRenderTarget }) {
    ...
    class Simulation {
      ...
      update() {
        ...
        this.advection.updateWithOptions({ dt: this.options.dt, isBounce: this.options.isBounce, BFECC: this.options.BFECC });
        this.externalForce.updateWithOptions({
          cursor_size: this.options.cursor_size,
        ...
        if (this.options.isViscous) {
          vel = this.viscous.updateWithOptions({
            viscous: this.options.viscous,
            iterations: this.options.iterations_viscous,
            dt: this.options.dt
          });
        }
        this.divergence.updateWithOptions({ vel: vel! });
        const pressure = this.poisson.updateWithOptions({ iterations: this.options.iterations_poisson });
        this.pressure.updateWithOptions({ vel: vel!, pressure: pressure! });
      }
    }
