...
const requestStatuses = ["pending", "in_review", "quoted", "accepted", "in_progress", "completed", "cancelled"] as const;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-inter text-xl font-bold text-gray-900 tracking-tight">Custom Design Requests</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left px-4 py-3 text-gray-700 font-semibold text-xs uppercase tracking-wide whitespace-nowrap">Budget</th>
              ...
              <tbody>
                {(requests as any[])?.map((request) => (
                  <tr key={request.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    {/* Budget */}
                    <td className="px-4 py-3 text-gray-700 whitespace-nowrap">
                      {request.budget ? `LKR ${Number(request.budget).toLocaleString()}` : "—"}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <Select
                        ...
                        onValueChange={(value) =>
                          updateStatus.mutate({ id: request.id, status: value as RequestStatus })
                        }
                      >
                        <SelectTrigger
                          className={`w-32 text-xs border-0 rounded-full font-medium ${
                            statusColors[request.status ?? "pending"] ?? "bg-gray-100 text-gray-700"
                          }`}
                        >
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent className="bg-white">
                          {requestStatuses.map((s) => (
                            <SelectItem key={s} value={s} className="text-gray-900 text-sm data-[highlighted]:bg-gray-100 data-[highlighted]:text-gray-900">
                              {statusLabels[s]}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      ...
                      <div className="flex gap-1.5">
                        ...
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button variant="outline" size="sm" className="h-8 w-8 p-0 border-gray-300 text-gray-600 hover:text-gray-900">
                              <Eye size={14} />
                            </Button>
                          </DialogTrigger>
                          <DialogContent className="max-w-lg bg-white">
                            <DialogHeader>
                              <DialogTitle className="text-lg font-semibold text-gray-900">Request Details</DialogTitle>
                            </DialogHeader>
                            <div className="space-y-3 text-sm">
                                ...
                                {request.budget && (
                                  <>
                                    ...
                                    <span className="text-gray-900">
                                      LKR {Number(request.budget).toLocaleString()}
                                    </span>
                                  </>
                                )}
                              ...
                              <div className="border-t border-gray-100 pt-3 space-y-2">
                                <p className="font-medium text-gray-600">Design Description</p>
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="outline" size="sm" className="h-8 w-8 p-0 border-red-200 text-red-500 hover:text-red-700 hover:bg-red-50">
                              <Trash2 size={14} />
                            </Button>
                          </AlertDialogTrigger>
                          <AlertDialogContent className="bg-white">
                            <AlertDialogHeader>
                              <AlertDialogTitle className="text-gray-900">Delete Request</AlertDialogTitle>
